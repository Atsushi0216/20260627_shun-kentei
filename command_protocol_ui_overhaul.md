...
<section id="start-screen" class="active">
    ...
    <main class="menu-box">
        <div class="hero">
            ...
            <p class="hero-sub">いちじく・桃農家アツシが出す18問で<br>あなたの<strong>食リテラシー偏差値</strong>を測ります。</p>
        </div>
...
## Phase 3: 結果画面＋PNGカードの改良

### 3-A. PNGカードの色調・レイアウト改良

`app.js` の PNGカード描画関数（`generateResultCard` または該当関数）で:

1. **背景グラデーション**: ダーク→暖色のグラデーション（上: `#faf7f2` → 下: `#f3efe6`（ライト時）、上: `#231e17` → 下: `#1a1510`（ダーク時））
2. **食タイプ名をPNGカード上で大きく表示**（偏差値の下、レーダーの上）
3. **農家の検定シリーズ署名を目立つ位置に**: カード下部に `「🌱 農家の検定シリーズ — いちじく・桃農家アツシ」` を追加
4. **カードタイトル横にも果物絵文字**: `🍑 食の目利き検定` として描画
...
- カード最下部のクレジット行にシリーズ署名を追加:
  ```javascript
  cardCtx.fillText('🌱 農家の検定シリーズ — いちじく・桃農家アツシ', cardW / 2, /* 下部Y座標 */);
  ```
...
- [ ] PNGカードに「🌱 農家の検定シリーズ — いちじく・桃農家アツシ」署名がある
- [ ] カードタイトルに🍑が付いている
- [ ] Xシェアテキストにタイプ名が含まれる
...
```json
{
  ...
  "text": "○○は△△である。",
  "correct": "good | bad",
  "explanation": "農家アツシの解説（畑のひとことトーン）",
  "difficulty": 1 | 2
}
```
